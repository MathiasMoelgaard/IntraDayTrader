...
class tcn():
    def __init__(self, moments, data_path = None, batch_size = None, input_dims = 6,
                  trainset = 100, loadModel = None, reporducability = False):
        if reporducability:
            np.random.seed(2020)
        ...
        for i, data_pt in enumerate(self.data):
            ...
            for j, value in enumerate(data_pt):
                if (i == 0):
                    ...
                    if (j < 1):
                        inner_data.append(np.log(value / self.data[i - 1][1]))
                    else:
    ...
    def test(self):
        results = self.m.evaluate(self.x_test, self.y_test, batch_size=128)
